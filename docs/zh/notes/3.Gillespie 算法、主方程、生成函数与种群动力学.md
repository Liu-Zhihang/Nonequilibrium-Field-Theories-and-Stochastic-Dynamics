...
![](https://files.mdnice.com/user/30198/3bec63e5-dd99-4fe2-bc89-6f70ba2687f5.jpg)
...
## 1.2 模拟的困境：固定时间步长法的局限

有了泊松过程的解析描述，一个自然而然的问题是：我们如何在计算机上模拟这样的过程？一个最直观的方法是**固定时间步长算法**(Fixed time step algorithm)。

![课程展示PPT](https://files.mdnice.com/user/30198/4f1ba484-27a2-43d8-9387-83a5db3f60da.png)
...
![课程展示PPT](https://files.mdnice.com/user/30198/f75b6ff9-c643-4a31-9b67-5ac3f34b7020.png)
...
![课程展示PPT](https://files.mdnice.com/user/30198/ae38f836-f653-4721-b943-2634dc9de086.png)
...
def gillespie_poisson(nu, t_max):
    ...
    参数:
    nu (float): 事件发生的速率 (rate)。
    t_max (float): 模拟的总时长。

    返回:
    tuple: 包含两个列表，分别是事件发生的时间点和对应的事件计数值。
    """
    # 初始化
    t = 0.0  # 当前时间
    n = 0    # 当前事件计数
    ...
    # 模拟循环，直到达到最大时间
    while t < t_max:
        # 1. 从 (0, 1] 区间抽取一个均匀分布的随机数
        #    注意：xi 不能为 0，因为 log(0) 是负无穷大。
        #    np.random.random() 生成 (0, 1) 区间内的随机数，符合要求。
        xi = np.random.random()
...
plt.show()
```
![运行代码输出](https://files.mdnice.com/user/30198/7d080bb1-a706-4934-8cc3-ab7732cb265c.png)
...
![](https://files.mdnice.com/user/30198/230e1dcf-63c8-475c-b840-3cd8b548d768.jpg)
...
## 5.4 Python 代码实现：Gillespie 模拟死亡过程
...
def gillespie_death_process(lam, n0, t_max):
    """
    使用 Gillespie 算法模拟简单死亡过程。

    参数:
    lam (float): 人均死亡速率 (per capita death rate)。
    n0 (int): 初始种群大小。
    t_max (float): 模拟的总时长。

    返回:
    tuple: 包含两个列表，分别是事件发生的时间点和对应的种群大小。
    """
    # 初始化
    t = 0.0
    n = n0
...
plt.show()
```
![运行代码输出](https://files.mdnice.com/user/30198/2544ec66-8794-4a40-b84c-1ed5e3a7e8cc.png)
...
通过引入状态依赖的速率，我们已经初步窥见了如何将这一框架应用于更复杂的系统，如化学反应网络和更真实的生态模型。期待后续课程！

关注公众号，回复【**Gillespie 算法**】到目前为止3节课的python 代码

需要说明的是，有读者在后台私信希望获得讲义，但本人并没有讲义，都是先听课做笔记，在撰写公众号文章，原始笔记示例如下：

![](https://files.mdnice.com/user/30198/18cc2947-7f4d-40d0-ab59-51347e927534.png)
