...
1. **Symmetric random walk $(p = q = 1/2)$ (对称随机游走 $(p=q=1/2)$)**: In this case, $p - q = 0$, so $\langle X_k \rangle = 0$. The particle has equal probability of moving left and right, with no directional preference. On average, the particle always stays near the origin. This is a **pure diffusion (纯扩散(pure diffusion))** process.
