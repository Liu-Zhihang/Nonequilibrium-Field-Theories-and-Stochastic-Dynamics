...
1. We started from the **Chapman-Kolmogorov equation** describing finite-time evolution.

2. By taking the infinitesimal time limit, we derived the **forward master equation** describing instantaneous probability changes, which is a differential equation.

3. We encapsulated all transition rate information of the system in a core mathematical object—the **Q matrix**—which became the "engine" driving probability evolution.

4. Through analyzing the elements of the Q matrix, we interpreted the master equation as a physically intuitive **gain-loss balance**, which is essentially a continuity equation for probability.

5. We applied this theoretical framework to a specific **linear birth-death process**, demonstrating how to construct the Q matrix based on microscopic rules and understand the relationship between its structure and system dynamics.

6. Finally, we explored the system's long-term behavior and introduced the **Perron-Frobenius theorem**, revealing how the topological structure of the state space (irreducibility) guarantees that the system can reach a unique, physically reasonable steady-state equilibrium.
