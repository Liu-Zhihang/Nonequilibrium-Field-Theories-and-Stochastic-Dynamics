...
Based on the above definitions, we can write down the master equations describing the evolution of $P_+$ and $P_-$ densities. These are a pair of coupled partial differential equations. For right-movers:
$$
...
The lecture points out that the effective particle flux $J_{\text{eff}}$ can be rewritten in a form commonly found in equilibrium transport theory:
$$
...
With the effective free energy functional, the entire system's dynamical equation can be elegantly written in the form of a **gradient flow**:
$$
