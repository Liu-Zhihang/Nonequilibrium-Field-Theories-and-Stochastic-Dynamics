...
1. **Spatial Continuization (空间的连续化)**: We will generalize the random walk model from a fixed, discrete lattice to continuous space. This step will naturally guide us from the binomial distribution to the Gaussian distribution, thus establishing the **Gaussian Random Walk (GRW) (高斯随机游走（Gaussian Random Walk, GRW）)** model.

2. **Temporal Randomization (时间的随机化)**: We will break the constraint that "time steps" are fixed and deterministic, and instead explore scenarios where events occur randomly on a continuous time axis. This will lead to the **Poisson Process (泊松过程（Poisson Process）)**, which is crucial in statistical physics and biology.
